...
-- Identifying UDPs that are not exist in target Vault
Declare @column_name nvarchar(100)
Declare @Select_Statement nvarchar(200)
Declare c Cursor For SELECT column_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'Files' and LEFT(column_name,4) = 'UDP_'
Open c
Fetch next from c into @column_name
While @@FETCH_STATUS=0 Begin
       Select @Select_Statement = 'UPDATE Files SET Validation_Status = ''ERROR'', Validation_Comment = CONCAT (''Invalid UDP Name: '+ @column_name +'; '', Validation_Comment) where [' + @column_name + '] is not null'
